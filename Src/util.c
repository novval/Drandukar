...
void readCommand(void) {
    ...
    if (inIdx == CONTROL_ADC) {
      ...
      cruiseControl(button1);                                           // Cruise control activation/deactivation
           ...
           key1 = !HAL_GPIO_ReadPin(BUTTON3_PORT, BUTTON3_PIN);
           key2 = !HAL_GPIO_ReadPin(BUTTON4_PORT, BUTTON4_PIN);

          if (!key1 && !key2){                             //если вкл задняя
             reverse_on = 1;
               if(reverse){
                 rtP_Left.n_max = rtP_Right.n_max = N_Rew_Max << 4; //скорость не более максимальной для задней
               }
               else{}
             }
          else{reverse_on = 0;}
          ...
          if(!reverse){
           if (key1 && key2){                                    //если вкл (1)          
             rtP_Left.n_max = rtP_Right.n_max = N_Low_Max << 4; //скорость не более максимальной для (1)
             }
           else if (key1 && !key2){                              //если вкл (2) 
             rtP_Left.n_max = rtP_Right.n_max = 2000 << 4; //скорость не более максимальной для (2)
             }
           else if (!key1 && key2){                               //если вкл (3)
             rtP_Left.n_max = rtP_Right.n_max = N_Mid_Max << 4;  //скорость максимальная (3)
             }
           else{}
          }
          else{}     
