  ...
  while(1) {
    ...
    engineSoundSimulation(input2[inIdx].cmd);
      ...
      if (inIdx == CONTROL_ADC) {                                   // Only use use implementation below if pedals are in use (ADC input)
        if (speedAvgAbs < 20) {                                     // Check if Hovercar is physically close to standstill to enable Double tap detection on Brake pedal for Reverse functionality
      ...
      // ####### VARIANT_HOVERCAR #######
       ...
       if (inIdx == CONTROL_ADC) {               // Only use use implementation below if pedals are in use (ADC input)
        ...
        #ifdef SUPPORT_BUTTONS
         if (!reverse) {                         // Check driving direction
    ...
    if ((TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF && speedAvgAbs < 20) || (batVoltage < BAT_DEAD && speedAvgAbs < 20)) {  // poweroff before mainboard burns OR low bat 3
      ...
      beepCount(0, 10, 30);
    } else if (BEEPS_BACKWARD && ((speed < -50 && speedAvg < 0) || MultipleTapBrake.b_multipleTap || reverse)) { // 1 beep fast (high pitch): Backward spinning motors
      beepCount(0, 5, 1);
      backwardDrive = 1;
    } else {  // do not beep
      beepCount(0, 0, 0);
    ...
    // ledToggle();                 // This is to measure the main() loop duration with an oscilloscope connected to LED_PIN
    // Update states
    inIdx_prev = inIdx;
